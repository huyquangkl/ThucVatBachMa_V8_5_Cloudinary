...
def save_image_file(file_storage):
    """Upload 1 file ảnh lên Cloudinary và trả về public_id (hoặc None)."""
    if not file_storage or not file_storage.filename:
        return None
    if not allowed_file(file_storage.filename):
        return None

    public_id_base = secure_filename(
...
def uploaded_file(filename):
    """Trả ảnh từ Cloudinary (hoặc redirect thẳng nếu là URL tuyệt đối)."""
    # Nếu DB đã lưu full URL (http...) thì redirect luôn
    if filename.startswith("http://") or filename.startswith("https://"):
        return redirect(filename)

    # Mặc định: filename chính là public_id Cloudinary (có thể kèm folder)
    url, _ = cloudinary_url(filename, secure=True)
    return redirect(url)
